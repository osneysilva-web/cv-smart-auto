...
import { getAdminStats, getAllMembersWithPayments, updateMemberStatus, deleteMember, manualApprovePayment, resetPaymentStatus } from '../services/databaseService';
import { LayoutDashboard, Users, FileText, TrendingUp, LogOut, DollarSign, Calendar, Loader2, Search, Trash2, Clock, Unlock, CheckCircle } from 'lucide-react';
...
export const AdminDashboard: React.FC<Props> = ({ onLogout }) => {
  ...
  const handleManualApprove = async (userId: string, memberId: number) => {
      if (!userId) return alert("Erro: ID de usuário não encontrado.");
      
      try {
          await manualApprovePayment(userId);
          setMembers(prev => prev.map(m => m.id === memberId ? { ...m, payment_status: 'paid' } : m));
          console.log(`Download liberado com sucesso para o usuário: ${userId}`);
      } catch (e: any) {
          const msg = e.message || "Erro desconhecido no servidor.";
          alert(`Falha ao liberar acesso: ${msg}`);
          console.error("Erro detalhado:", e);
      }
  };

  const handleResetPayment = async (userId: string, memberId: number) => {
      if (!userId) return alert("Erro: ID de usuário não encontrado.");
      if (!window.confirm("Deseja cobrar novamente este usuário? O botão voltará para 'Liberar Download'.")) return;
      
      try {
          await resetPaymentStatus(userId);
          setMembers(prev => prev.map(m => m.id === memberId ? { ...m, payment_status: 'pending' } : m));
      } catch (e: any) {
          const msg = e.message || "Erro desconhecido no servidor.";
          alert(`Falha ao resetar status: ${msg}`);
      }
  };

  const handleDeleteMember = async (id: number) => {
      if (!window.confirm("Confirmar exclusão definitiva do utilizador?")) return;
      try {
                                        ...
                                        <tr key={member.id} className="hover:bg-slate-50/50 transition-colors">
                                            ...
                                            <td className="px-8 py-6">
                                                <span className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest border ${
                                                    ...
                                                    {member.payment_status === 'paid' ? 'PAGO' : 'PENDENTE'}
                                                </span>
                                            </td>
                                            <td className="px-8 py-6">
                                                <div className="flex items-center justify-center gap-3 relative z-30">
                                                    {member.payment_status === 'paid' ? (
                                                        <button 
                                                            onClick={(e) => { e.stopPropagation(); handleResetPayment(member.user_id, member.id); }} 
                                                            className="flex items-center gap-2 px-5 py-2.5 bg-green-100 text-green-700 border-2 border-green-200 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-red-50 hover:text-red-600 hover:border-red-100 transition-all shadow-sm cursor-pointer"
                                                        >
                                                            <CheckCircle size={14} /> APROVADO
                                                        </button>
                                                    ) : (
                                                        <button 
                                                            onClick={(e) => { e.stopPropagation(); handleManualApprove(member.user_id, member.id); }} 
                                                            className="flex items-center gap-2 px-5 py-2.5 bg-blue-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-700 transition-all shadow-md cursor-pointer"
                                                        >
                                                            <Unlock size={14} /> LIBERAR DOWNLOAD
                                                        </button>
                                                    )}
                                                    <button 
