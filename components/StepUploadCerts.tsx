...
// 🔥 Compressão de imagem (anti-timeout / anti-limite Vercel)
...
export const StepUploadCerts: React.FC<Props> = ({
  onNext,
  onSkip,
  onBack,
  userId,
  ...
  // ✅ Upload seguro (limite + compressão)
  const handleFileChange = async (
    ...
    for (const file of selectedFiles) {
      if (!file.type.startsWith('image/')) {
        alert(`O ficheiro ${file.name} não é uma imagem`);
        continue;
      }
  ...
  // ✅ Processamento protegido (Vercel-safe)
    ...
    try {
      if (userId) {
        for (const file of files) {
          await uploadUserDocument(file, userId, 'CERTIFICATE');
        }
      }
      ...
      alert('Falha ao processar documentos. Verifique a qualidade das imagens.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="max-w-md mx-auto bg-white p-10 rounded-[40px] shadow-2xl border border-slate-200 mt-12">
      <div className="mb-8 text-center">
        ...
        <p className="text-slate-600 text-sm font-medium">
          Adicione certificados para enriquecer seu currículo.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-10">
        {files.map((file, i) => (
          <div
            key={i}
            className="relative bg-slate-50 p-4 rounded-2xl border-2 border-slate-100 text-[10px] font-black text-slate-900 flex flex-col items-center gap-2"
          >
            <FileBadge size={24} className="text-blue-600 mb-1" />
            <span className="truncate w-full text-center uppercase">
              {file.name}
            </span>
            <button
              onClick={() => removeFile(i)}
              className="absolute -top-2 -right-2 bg-slate-900 rounded-full p-2 shadow-lg text-white hover:bg-red-600 transition-colors"
            >
        ...
        <label className="border-2 border-dashed border-slate-300 rounded-2xl p-6 flex flex-col items-center justify-center text-center hover:bg-slate-50 cursor-pointer min-h-[100px] transition-all group">
          ...
          />
          <Plus
            className="text-slate-800 mb-1 group-hover:scale-110 transition-transform"
            size={32}
            strokeWidth={3}
          />
